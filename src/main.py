...
def experiment_process(data, models, param_grids):
    ...
    stats_hyper = hyperparameter_tuning(models, param_grids , data["X_train"], data["y_train"] )
...
def process(config, mode="experiment"):
    ...
    fill_values = {"Age" : 14, "Embarked" : "U"}

    #Information for modelling stages
    param_grids = {"LogisticRegression":{"C":np.logspace(0, 4, 10), "penalty": ["l1","l2"]}}

    df_train_clean = clean(df_train, fill_values=fill_values, columns_to_drop=columns_to_drop, maps=maps)
    df_test_clean = clean(df_test, fill_values=fill_values, columns_to_drop=columns_to_drop, maps=maps)
